...
-- pgrest-lambda reads the key from pg_constraint and embedding follows:
-- GET /rest/v1/todos?select=*,categories(name)
-- DSQL accepted REFERENCES from 2026-08-27. Before that this column carried
-- no key and the relationship was inferred from its {singular}_id name, which
-- still works when no key is declared.
```
...
- Foreign keys work, with one restriction: `REFERENCES` inside `CREATE TABLE` is accepted, but a key added to a table that already exists has to be `ALTER TABLE ... ADD CONSTRAINT ... FOREIGN KEY ... NOT VALID` — plain `ADD CONSTRAINT` and `VALIDATE CONSTRAINT` are rejected. `NOT VALID` still enforces every write after it, so declare the key in `CREATE TABLE` where you can
- **Name a key column `{singular_table}_id`** anyway: with no key declared, that name is all pgrest-lambda has to infer the relationship from (e.g., `customer_id` → `customers` table, `category_id` → `categories`, `address_id` → `addresses`)
