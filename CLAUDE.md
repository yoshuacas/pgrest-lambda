...
│   │       └── cognito.mjs    # Amazon Cognito implementation
...
8. **Auth providers are swappable** — Cognito is the default. The provider interface in `src/auth/providers/interface.mjs` defines the contract. Future providers (Auth0, etc.) must implement the same interface.
