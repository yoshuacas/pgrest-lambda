...
| `PGREST_DB_TX_END` | `db-tx-end` | `commit` | How the transaction a write runs in ends, and whether the request may say otherwise: `commit`, `rollback`, `commit-allow-override`, `rollback-allow-override`. Under a `rollback` ending the response is the one the write earned — same status, same body, same `Content-Range` — and the rows are then undone. The `-allow-override` forms let a request invert the ending with `Prefer: tx=commit` or `Prefer: tx=rollback`, echoed in `Preference-Applied`; without them the preference is ignored and not echoed. Reads never open a transaction, because they have nothing to undo. |
