...
The numbers on this page come from `conformance/cedar/results/latest.json` (run `2026-08-28T21:21:02Z`, commit `4cebc95`, target `dsql`) — the same tree as the published PostgREST run, re-measured on it after Aurora DSQL took foreign key constraints. Seven earlier runs are kept beside it, and every difference between them is code or case count, not noise:
